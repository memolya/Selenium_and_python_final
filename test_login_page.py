...
def test_guest_can_go_to_login_page_and_see_login_page(browser):
    ...
    main_page = MainPage(browser, main_link) # Гость открывает главную страницу
    main_page.open()
    main_page.go_to_login_page() # Переходит по ссылке логина
    login_page = LoginPage(browser, browser.current_url) # Создаём PageObject для страницы логина
